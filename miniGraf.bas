  ...
  290 IF INKEY(-94) AND NOT INKEY(-5) AND NOT INKEY(-6) THEN GOSUB 7120:pen%=(pen%+1) MOD 64:GOSUB 2000
  300 IF INKEY(-24) AND NOT INKEY(-5) AND NOT INKEY(-6) THEN GOSUB 7120:pen%=(pen%-1):GOSUB 2230:GOSUB 2000
  310 IF INKEY(-94) AND NOT INKEY(-5) AND INKEY(-6) THEN GOSUB 7120:paper%=(paper%+1) MOD 64:GOSUB 2150
  320 IF INKEY(-24) AND NOT INKEY(-5) AND INKEY(-6) THEN GOSUB 7120:paper%=(paper%-1):GOSUB 2260:GOSUB 2150
  ...
  420 IF INKEY(-90) THEN cpt%=0:GOSUB 2150:GOSUB 7030:GOSUB 7150
  430 IF INKEY(-30) THEN MODE 1:VDU 23,1,1:GOSUB 7150:END
  440 memc%=POINT(xc%,yc%):FOR i%=1 TO 63:GCOL 0,i%:PLOT 69,xc%,yc%:NEXT i%:GCOL 0,memc%:PLOT 69,xc%,yc%
  450 GOTO 210
 2000 REM ********** change pen
 2010 GCOL 0,pen%:COLOUR 15:GOSUB 2120:PRINT TAB(0,17);"Pen:";STR$(pen%);" - Paper:";STR$(paper%)
 2020 RETURN
 2030 REM ********** help
 ...
 2280 RETURN
 2290 REM ********** show infos
 2300 GOSUB 2120:PRINT TAB(0,17);STR$(cpt%);" dots. Pen:";STR$(pen%)+" Paper:";STR$(paper%);" Steps:"+STR$(stp%)
 2310 RETURN
